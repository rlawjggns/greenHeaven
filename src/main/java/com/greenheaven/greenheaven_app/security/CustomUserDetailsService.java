...
public class CustomUserDetailsService implements UserDetailsService { // UserDetailsService 구현, Spring Security는 인증 과정에서 UserDetailService를 사용하여 사용자 정보를 로드
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException { 
        ...
        return new UserPrincipal(user); // 조회한 User 엔티티를 기반으로 UserPrincipal 객체를 생성해 반환, UserPrincipal은 UserDetail 인터페이스를 구현한 클래스이며, 시큐리티가 사용자 인증 및 권한 부여를 위해 필요한 정보를 제공해줌
    }
}
