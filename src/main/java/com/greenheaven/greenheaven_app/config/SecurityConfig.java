package com.greenheaven.greenheaven_app.config;

import com.greenheaven.greenheaven_app.security.CustomUserDetailsService;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception { // HTTP 요청 보안을 구성하기 위한 빌더 객체를 인자로 받아 설정을 시작 
        http
                ...
                .authorizeHttpRequests(auth -> auth // HTTP 요청에 대한 접근 제어 설정
                        .requestMatchers("/", "/user/login", "/user/signup", "/user/password/reset", "/user/password/reset/confirm","/error","/css/**", "/js/**", "/images/**", "/weather").permitAll() // 괄호 안의 URL 패턴(홈, 로그인, 회원가입, 에러 페이지, 정적 리소스 경로 등)은 인증 없이 접근 가능하게 허용
